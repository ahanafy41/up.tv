    ...
    local delegate = View.AccessibilityDelegate({
        performAccessibilityAction = function(host, action, args)
            if action == AccessibilityNodeInfo.ACTION_SCROLL_FORWARD then
                fwdFunc()
                return true
            elseif action == AccessibilityNodeInfo.ACTION_SCROLL_BACKWARD then
                bwdFunc()
                return true
            end
...
function TTSManager.init()
    TTSManager.tts = TextToSpeech(activity, TextToSpeech.OnInitListener{
        onInit = function(status)
            if status == TextToSpeech.SUCCESS then
                local result = TTSManager.tts.setLanguage(Locale("ar"))
                if result == TextToSpeech.LANG_MISSING_DATA or result == TextToSpeech.LANG_NOT_SUPPORTED then
                    -- Arabic not supported, try default
                    TTSManager.tts.setLanguage(Locale.getDefault())
                end
...
function speak(text)
    if TTSManager.isInitialized and text and text ~= "" then
        pcall(function()
            if Build.VERSION.SDK_INT >= 21 then
                TTSManager.tts.speak(text, TextToSpeech.QUEUE_FLUSH, nil, "id_"..os.time())
            else
                TTSManager.tts.speak(text, TextToSpeech.QUEUE_FLUSH, nil)
            end
        end)
    end
end
...
function VideoPlayer.sendNotification(title, isPlaying)
    -- Disabled to prevent screen reader interruptions and status bar clutter
end
...
function VideoPlayer.showVideoUI()
    ...
    makeSeekBarAccessible(vSeek, function() VideoPlayer.seekForward() end, function() VideoPlayer.seekRewind() end)
...
function AudioPlayer.showUI()
    ...
    makeSeekBarAccessible(pSeek, function() AudioPlayer.seekForward() end, function() AudioPlayer.seekRewind() end)
